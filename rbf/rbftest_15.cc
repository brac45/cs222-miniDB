//
// Created by Seoyeon Hwang on 10/28/19.
//
...
int RBFTest_Update2(RecordBasedFileManager &rbfm) {
    // Functions tested
    ...
    std::string shortStr2;
    for (int i = 0; i < 10; i++) {
        shortStr2.push_back('t');
    }
    ...
    // update short record with same length short record
    updateRecord(rbfm, shortRID, shortStr2);

    //read updated short record and verify its content
    readRecord(rbfm, shortRID, shortStr2);

    // update short record with longer record
    updateRecord(rbfm, shortRID, midString);

    //read updated short record and verify its content
    readRecord(rbfm, shortRID, midString);

    // update mid record with shorter record
    updateRecord(rbfm, midRID, shortStr2);

    //read updated mid record and verify its content
    readRecord(rbfm, midRID, shortStr2);



    // update short record with mid record again
    updateRecord(rbfm, midRID, midString);
    ...
    readRecord(rbfm, shortRID, midString);
    readRecord(rbfm, midRID, midString); //여기서 터짐;;
...
int main() {
    ...
    remove("test_update");
    return RBFTest_Update2(RecordBasedFileManager::instance());
}